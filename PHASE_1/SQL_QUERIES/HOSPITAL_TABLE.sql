...
-- Ethnicity
SELECT p.PAT_ID
	,STRING_AGG(eth_c.NAME, '|') AS ETHNICITY -- STIRNG_AGG for SQL SERVER 2017 and later
INTO #ethincity_p
FROM PATIENT p
...
SELECT enc.PAT_ID AS pid
	...
	,MIN(DATEDIFF(DAY, BIRTH_DATE, HOSP_ADMIT_DT)) AS ageAtAdmission_Anonymized
	,MIN(HOSP_DC_DT) AS HOSP_DC_DT
	,MIN(DATEDIFF(SECOND, HOSP_ADMIT_DT, HOSP_DC_DT)) AS hospitalDischarge_Anonymized
	,MIN(RACE) AS RACE
